  ...
  return (
    <span
      className={cn(
        ...
        active
          ? onDarkHeader
            ? 'bg-white/82 shadow-[inset_0_1px_0_rgba(255,255,255,0.85),0_3px_8px_rgba(0,0,0,0.18)]'
            : 'bg-white shadow-[inset_0_1px_0_rgba(255,255,255,0.95),0_2px_6px_rgba(39,56,47,0.12)]'
          : onDarkHeader
...
export function LibraryToggle({ isInternal, variant }: LibraryToggleProps) {
  ...
  const onDarkHeader = variant === 'sidekick'
  ...
  return (
    <div className={shellClass} aria-label="Library switcher">
      <Link href="/" aria-label="Sidekick Prompt Library" className="rounded-full focus-ring">
        <ToggleIcon src="/logos/sidekick_logo.svg" alt="Sidekick" active={sidekickActive} onDarkHeader={onDarkHeader} />
      </Link>
      <Link href="/skills" aria-label="Claude Skills Library" className="rounded-full focus-ring">
        <ToggleIcon src="/logos/Claude_AI_symbol.svg" alt="Claude Skills" active={claudeActive} onDarkHeader={onDarkHeader} />
      </Link>
    </div>
  )
}
